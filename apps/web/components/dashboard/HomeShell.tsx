...
export default function HomeShell({ address, profile }: { address: string; profile: PublisherProfile }) {
  ...
  useEffect(() => {
    setOnboardingCompleted(hasCompletedHomeOnboarding(address));
  }, [address]);

  function handleOnboardingComplete(reason: OnboardingCompletionReason) {
    completeHomeOnboarding(address, reason);
    ...
    <WorkspaceFrame address={address} profile={profile} title="Home">
      <main className="p-6 md:p-10">
        <div className="mx-auto max-w-[76rem]">
          {onboardingCompleted === null ? <div aria-busy="true" className="min-h-[20rem]" /> : onboardingCompleted ? <ReturningHome profile={profile} /> : <HomeFirstRun address={address} onComplete={handleOnboardingComplete} />}
        </div>
      </main>
    </WorkspaceFrame>
  );
...
function ReturningHome({ profile }: { profile: PublisherProfile }) {
  return (
    <>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <Link href="/app/purchases" className="rounded-[16px] border border-[#262626] p-5 no-underline hover:border-[#444]"><p className="text-[10px] uppercase tracking-[0.12em] text-[#666]">Library</p><p className="mt-2 text-[15px] text-[#d0d0d0]">View your purchases →</p></Link>
        <Link href="/app/analysis" className="rounded-[16px] border border-[#262626] p-5 no-underline hover:border-[#444]"><p className="text-[10px] uppercase tracking-[0.12em] text-[#666]">Publisher</p><p className="mt-2 text-[15px] text-[#d0d0d0]">Open analysis →</p></Link>
      </div>
    </>
  );
}
