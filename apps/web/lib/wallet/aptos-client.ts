...
/**
 * Sign and submit an Aptos transaction via the connected wallet (AIP-62
 * `aptos:signAndSubmitTransaction`, v1.1.0). The wallet (user) pays the gas
 * fee — this is the Fase D payment flow: buyer signs, buyer pays gas.
 */
export async function signAndSubmitTransaction(
  ...
  const signAndSubmit = wallet.features['aptos:signAndSubmitTransaction']?.signAndSubmitTransaction;
  if (!signAndSubmit) {
